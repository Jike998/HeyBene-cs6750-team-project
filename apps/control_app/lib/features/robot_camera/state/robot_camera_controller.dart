...
class RobotCameraController extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Future<void> startFromUi() async {
    if (!state.connection.usbConnected || state.mode != RobotMode.drive) {
      state.setGamepadDebug(
        'START blocked: usb=${state.connection.usbConnected}, mode=${state.mode.name}',
      );
      await _sendStatus();
      return;
    }
    ...
    state.setGamepadDebug('Drive armed from app START (RTR_520)');
    await _sendStatus();
  }

  Future<void> stopFromUi() async {
    state.setGamepadDebug('Drive stopped from app STOP');
    await _stopActiveDrive(disarm: true);
    await _sendStatus();
  }
  ...
  Future<void> _handleGamepadEvent(Map<String, dynamic> event) async {
    ...
    state.setGamepadDebug('Gamepad event: $eventType');

    // We intentionally do NOT use gamepad buttons to arm/disarm.
    // Arming is controlled by the app UI START/STOP button.
    if (eventType == 'button') {
      final button = event['button'];
      final pressed = event['pressed'];
      state.setGamepadDebug('Button: $button pressed=$pressed');
      return;
    }
    ...
    if (!state.connection.usbConnected || state.mode != RobotMode.drive || !state.driveArmed) {
      return;
    }
